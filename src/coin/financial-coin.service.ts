...
export class FinancialCoinService {
  ...
  async getMyBalance(userId: string) {
    ...
    return {
      ...
      account: {
        ...data,
        balance: this.financial.unitsToDisplay(data.balance_units),
        balance_units: String(data.balance_units),
      },
    };
  }

  async createRechargeRequest(requesterId: string, targetUserId: string, amount: number, idempotencyKey?: string) {
    if (requesterId === targetUserId) throw new BadRequestException('Requester and target cannot be the same user');
    return {
      success: true,
      request: await this.financial.createRechargeRequest(requesterId, targetUserId, amount, idempotencyKey),
    };
  }
